import { motion, useMotionValue, useTransform, PanInfo } from 'framer-motion';
import { MapPin, Music } from 'lucide-react';
import { cn } from '@/lib/utils';
import { formatDistance } from '@/services/geolocation';
import { Progress } from '@/components/ui/progress';

interface ClubSwipeCardProps {
  club: {
    venue_name: string;
    genres: string[];
    capacity: number;
    distance: number | null;
    image_url: string;
    heat: number;
    nextEvent: { title: string; id: string } | null;
    eventCount: number;
  };
  onSwipe: (direction: 'left' | 'right' | 'up') => void;
  isTop?: boolean;
}

export const ClubSwipeCard = ({ club, onSwipe, isTop = false }: ClubSwipeCardProps) => {
  const x = useMotionValue(0);
  const y = useMotionValue(0);

  const rotate = useTransform(x, [-200, 200], [-25, 25]);
  const favOpacity = useTransform(x, [0, 100], [0, 1]);
  const skipOpacity = useTransform(x, [-100, 0], [1, 0]);
  const superOpacity = useTransform(y, [-100, 0], [1, 0]);

  const handleDragEnd = (_event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo) => {
    const threshold = 100;
    if (info.offset.x > threshold) onSwipe('right');
    else if (info.offset.x < -threshold) onSwipe('left');
    else if (info.offset.y < -threshold) onSwipe('up');
  };

  const maxHeat = 100;
  const heatPercent = Math.min(100, (club.heat / maxHeat) * 100);

  return (
    <motion.div
      className={cn(
        'absolute w-full aspect-[3/4] rounded-3xl overflow-hidden',
        isTop ? 'cursor-grab active:cursor-grabbing z-10' : 'z-0'
      )}
      style={{
        x: isTop ? x : 0,
        y: isTop ? y : 0,
        rotate: isTop ? rotate : 0,
        scale: isTop ? 1 : 0.95,
        filter: isTop ? 'none' : 'blur(2px)',
      }}
      drag={isTop}
      dragConstraints={{ left: 0, right: 0, top: 0, bottom: 0 }}
      dragElastic={0.9}
      onDragEnd={handleDragEnd}
    >
      {/* Background */}
      {club.image_url ? (
        <img
          src={club.image_url}
          alt={club.venue_name}
          className="absolute inset-0 w-full h-full object-cover"
          draggable={false}
        />
      ) : (
        <div className="absolute inset-0 w-full h-full bg-gradient-to-br from-primary/30 to-accent/30 flex items-center justify-center">
          <span className="text-7xl">🏢</span>
        </div>
      )}

      <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent" />

      {/* Distance Badge */}
      {club.distance !== null && (
        <div className="absolute top-4 left-4 px-3 py-1.5 bg-black/60 backdrop-blur-sm rounded-full flex items-center gap-1.5">
          <MapPin className="w-3.5 h-3.5 text-primary" />
          <span className="text-xs font-semibold text-white">{formatDistance(club.distance)}</span>
        </div>
      )}

      {/* Heat Badge */}
      <div className="absolute top-4 right-4 px-3 py-1.5 bg-black/60 backdrop-blur-sm rounded-full">
        <span className="text-xs font-bold text-white">🔥 {club.heat}</span>
      </div>

      {/* Swipe Stamps */}
      {isTop && (
        <>
          <motion.div
            className="absolute top-8 left-8 px-6 py-2 border-4 border-green-500 text-green-500 font-bold text-2xl rounded-lg rotate-[-20deg]"
            style={{ opacity: favOpacity }}
          >
            FAVORITE ❤️
          </motion.div>
          <motion.div
            className="absolute top-8 right-8 px-6 py-2 border-4 border-destructive text-destructive font-bold text-2xl rounded-lg rotate-[20deg]"
            style={{ opacity: skipOpacity }}
          >
            SKIP
          </motion.div>
          <motion.div
            className="absolute top-8 left-1/2 -translate-x-1/2 px-6 py-2 border-4 border-accent text-accent font-bold text-2xl rounded-lg"
            style={{ opacity: superOpacity }}
          >
            SUPER ⭐
          </motion.div>
        </>
      )}

      {/* Club Info */}
      <div className="absolute bottom-0 left-0 right-0 p-6">
        <h2 className="text-2xl font-bold text-white mb-2 line-clamp-1">{club.venue_name}</h2>

        {/* Genre Pills */}
        <div className="flex items-center gap-1.5 mb-2 flex-wrap">
          <Music className="w-3.5 h-3.5 text-primary" />
          {club.genres.slice(0, 4).map((genre) => (
            <span
              key={genre}
              className="px-2.5 py-0.5 bg-primary/20 backdrop-blur-sm rounded-full text-[11px] text-primary-foreground font-medium"
            >
              {genre}
            </span>
          ))}
        </div>

        {/* Capacity */}
        <p className="text-xs text-white/70 mb-2">🏟️ {club.capacity} cap</p>

        {/* Heat Bar */}
        <div className="mb-2">
          <Progress value={heatPercent} className="h-1.5" />
        </div>

        {/* Next Event */}
        {club.nextEvent && (
          <p className="text-sm text-primary font-medium mb-1 line-clamp-1">
            Next: {club.nextEvent.title}
          </p>
        )}

        <p className="text-xs text-white/60">{club.eventCount} events</p>
      </div>
    </motion.div>
  );
};
